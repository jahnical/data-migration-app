...
export const migrationAsyncActions = {
    ...
    migrateTEIs: (teis, filteredTeis, targetOrgUnit, targetOrgUnitName, credentials, engine) => async (dispatch) => {
        if (!teis?.length || !targetOrgUnit || !engine || !filteredTeis?.length || !credentials) {
            throw new Error('Missing required parameters');
        }
        ...
        try {
            ...
            teis = filteredTeis.map(tei => teis.find(t => t.trackedEntityInstance === tei.id));
            const updatedTeis = teis.map(tei => {
                ...
                if (updatedTei.programOwners) {
                    updatedTei.programOwners = updatedTei.programOwners.map(ownership => ({
                        ...ownership,
            ...
            //     return customEngine.mutate(ownershipMutation);
            // });
