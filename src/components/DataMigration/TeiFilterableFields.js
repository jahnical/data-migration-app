import { VALUE_TYPE_DATE, VALUE_TYPE_DATETIME, VALUE_TYPE_INTEGER, VALUE_TYPE_INTEGER_NEGATIVE, VALUE_TYPE_INTEGER_POSITIVE, VALUE_TYPE_INTEGER_ZERO_OR_POSITIVE, VALUE_TYPE_NUMBER, VALUE_TYPE_TEXT } from '@dhis2/analytics'
...
const TeiFilterableFields = () => {
    ...
    const attributesToDisplay = useSelector(migrationSelectors.getMigrationAttributesToDisplay)
    ...
    const handleFieldClick = (field) => {
        ...
        if (filter) {
            if ([VALUE_TYPE_NUMBER, VALUE_TYPE_INTEGER,
                VALUE_TYPE_INTEGER_NEGATIVE, VALUE_TYPE_INTEGER_POSITIVE,
                VALUE_TYPE_INTEGER_ZERO_OR_POSITIVE].includes(field.valueType)) {
                setNumberRange({ min: filter.min, max: filter.max })
            } else if (field.valueType === VALUE_TYPE_DATE || field.valueType === VALUE_TYPE_DATETIME) {
    ...
    const isActiveFilter = (field) => filters.some(filter => filter.field === field.name)
    ...
    const applyFilters = () => {
        ...
        if ([VALUE_TYPE_NUMBER, VALUE_TYPE_INTEGER,
            VALUE_TYPE_INTEGER_NEGATIVE, VALUE_TYPE_INTEGER_POSITIVE,
            VALUE_TYPE_INTEGER_ZERO_OR_POSITIVE].includes(selectedField.valueType)) {
                handleFilterChange({
                    min: numberRange.min,
                    max: numberRange.max,
        ...
        } else if (selectedField.valueType === VALUE_TYPE_DATE || selectedField.valueType === VALUE_TYPE_DATETIME) {
            handleFilterChange({
    ...
    const renderFilterDialog = () => {
        ...
        return (
            ...
            <Box padding="8px 0">
                ...
                {( [VALUE_TYPE_NUMBER, VALUE_TYPE_INTEGER,
                VALUE_TYPE_INTEGER_NEGATIVE, VALUE_TYPE_INTEGER_POSITIVE,
                VALUE_TYPE_INTEGER_ZERO_OR_POSITIVE].includes(selectedField.valueType) ) && (
                <div>
                    ...
                    <InputField
                        ...
                        onChange={({ value }) => setNumberRange({...numberRange, min: value })}
                    />
                    <InputField
                        type="number"
                        label={i18n.t('Max value')}
                        value={numberRange.max}
                        onChange={({ value }) => setNumberRange({...numberRange, max: value })}
                    />
                    </Box>
                </div>
                )}
                {(selectedField.valueType === VALUE_TYPE_DATE || selectedField.valueType === VALUE_TYPE_DATETIME) && (
                <div>
                    <h5>{i18n.t('Select date range')}</h5>
                    <Box margin="8px 0">
                    <InputField
                        type={selectedField.valueType === 'DATE' ? 'date' : 'datetime-local'}
            ...
            <ModalActions>
                ...
                <Button onClick={() => applyFilters()} primary disabled={!(numberRange.min && numberRange.max) && !(dateRange.start && dateRange.end) && !keyword}>
                    {i18n.t('Apply Filter')}
                </Button>
                </ButtonStrip>
            </ModalActions>
        ...
        <div className={classes.container}>
            <Box padding="16px">
                <div className={classes.attributesWrapper}>
                    <span style={{ fontSize: '0.8em', padding: '16px', fontWeight: '500', color: 'grey' }} >{i18n.t('Filterable Fields:')}</span>
                    {fields.map((field) => (
                        <Chip
                            key={field.name}
