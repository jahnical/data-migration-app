...
const DataMigrationModal = ({ onClose }) => {
    ...
    const migrateData = () => {
        dispatch(migrationAsyncActions.migrateTEIs(allTeis, filteredTeis, targetOrgUnit, metadata[targetOrgUnit], credentials, engine))
    }
        ...
        <Modal onClose={onClose} position="middle" large>
            ...
            {loading ? <ModalContent>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px' }}>
                    <CircularLoader />
                    <span>{i18n.t('Migrating {{count}} TEIs...', { count: filteredTeis.length })}</span>
                </div>
            </ModalContent> : error ? <ModalContent><NoticeBox error title={i18n.t('Could not load TEIs')}>
                {error?.message ||
                    i18n.t(
                        "The TEIs couldn't be migrated. Try again or contact your system administrator."
                    )}
            </NoticeBox></ModalContent> : <>
                <ModalContent style={{
                    display: 'flex',
                    flexDirection: 'column'}}>
                   {migrationStatus === 'success' ? <NoticeBox success title={i18n.t('Migration successful')}>
                        {i18n.t('All TEIs have been successfully migrated to the selected org unit.')}
                    </NoticeBox> :  <><div style={{ marginBottom: '20px' }}>
                ...
                <ModalActions>
                    <ButtonStrip>
                        ...
                        <Button
                            disabled={!targetOrgUnit || !credentials.username || !credentials.password}
                            primary
                            onClick={migrateData}
